...
# VENDOR PATCH: ATACDataProcessor (pysam) and SpatialDataProcessor (pyvips, PIL, boto3)
# are imported lazily inside the two functions that use them. Upstream imports them at
# module scope, which forces those heavy deps on every conversion -- including plain
# scRNA datasets that touch neither code path. See scripts/h5ad_to_cxg/README.md.
...
def convert_uns_to_cxg_group(cxg_container, metadata_dict, dataset_version_id, group_metadata_name="uns", ctx=None):
    ...
    spatial_processor = None  # VENDOR PATCH: constructed lazily, only for spatial data
            ...
            if key == UNS_SPATIAL_KEY:
                for object_id, content in value.items():
                    if object_id != IS_SINGLE:
                        if spatial_processor is None:  # VENDOR PATCH: lazy import
...
def convert_coverage_to_cxg_array(
    ...
    from backend.layers.processing.utils.atac import ATACDataProcessor  # VENDOR PATCH: lazy import
